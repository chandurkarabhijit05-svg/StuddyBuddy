      ...
      <div className="relative flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-semibold text-white group-hover:text-slate-100 transition-colors">{label}</h4>
            <ChevronRight className="w-4 h-4 text-slate-600 group-hover:text-slate-400 group-hover:translate-x-1 transition-all" />
          </div>
          <p className="text-xs text-slate-500 mt-1 leading-relaxed group-hover:text-slate-400 transition-colors">{description}</p>
        </div>
      </div>
...
export default function QuickActions({ onRefresh, onClearSearch, onScrollUpload }) {
  const actions = [
    { icon: Upload, label: "Upload PDF", description: "Add new study materials to analyze", color: "blue", onClick: onScrollUpload },
    { icon: RotateCcw, label: "Refresh Data", description: "Reload and sync latest content", color: "emerald", onClick: onRefresh },
    { icon: SearchX, label: "Clear Search", description: "Reset filters and search queries", color: "amber", onClick: onClearSearch },
    { icon: ArrowUp, label: "Back to Top", description: "Quickly return to page header", color: "violet", onClick: () => window.scrollTo({ top: 0, behavior: "smooth" }) },
  ];
  ...
  return (
    <section className="w-full max-w-4xl mx-auto px-4 py-8">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
        ...
        <span className="flex items-center gap-1.5"><MousePointerClick className="w-3 h-3" /> Click to activate</span>
        <span className="flex items-center gap-1.5"><Sparkles className="w-3 h-3" /> Hover for preview</span>
      </motion.div>
    </section>
  );
}